...
pub struct VisitingEmitterModule<'a, 'b, 'c, 'd> {
...
impl<'a, 'b, 'c, 'd> VisitingEmitterModule<'a, 'b, 'c, 'd> {
    ...
    pub(crate) fn get_loc_info<'e>(&self, rule: &'e WhammProvider) -> Option<LocInfo<'e>> {
        if let Some(curr_instr) = self.app_iter.curr_op() {
            rule.get_loc_info_module(self.app_iter.module, curr_instr)
        } else {
            None
        }
    }
    ...
    pub(crate) fn emit_if_with_orig_as_else(
        ...
        let orig_ty_id = OpcodeEvent::get_ty_info_for_instr_module(
...
impl Emitter for VisitingEmitterModule<'_, '_, '_, '_> {
