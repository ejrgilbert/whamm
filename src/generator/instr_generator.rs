...
fn get_loc_info_module<'a>(
    rule: &'a WhammProvider,
    emitter: &VisitingEmitterModule,
) -> Option<LocInfo<'a>> {
...
fn get_loc_info_comp<'a>(
    rule: &'a WhammProvider,
    emitter: &VisitingEmitterComponent,
) -> Option<LocInfo<'a>> {
    // Pull the curr instr each time this is called to keep from having
    // long-lasting refs into self.emitter.
    emitter.get_loc_info(rule)
...
pub struct InstrGeneratorModule<'a, 'b, 'c, 'd, 'e> {
    pub emitter: VisitingEmitterModule<'a, 'b, 'c, 'd>,
    pub ast: SimpleAST,
...
impl<'a, 'b, 'c, 'd, 'e> InstrGeneratorModule<'a, 'b, 'c, 'd, 'e> {
    pub fn new(
        emitter: VisitingEmitterModule<'a, 'b, 'c, 'd>,
        ast: SimpleAST,
        err: &'e mut ErrorGen,
    ) -> Self {
        Self {
    ...
    pub fn run(&mut self) -> bool {
        ...
        while first_instr || self.emitter.next_instr() {
            ...
            rules.iter().for_each(|rule| {
                ...
                if let Some(loc_info) = get_loc_info_module(rule, &self.emitter) {
...
impl InstrGeneratorModule<'_, '_, '_, '_, '_> {
...
pub struct InstrGeneratorComponent<'a, 'b, 'c, 'd, 'e> {
    pub emitter: VisitingEmitterComponent<'a, 'b, 'c, 'd>,
    pub ast: SimpleAST,
    pub err: &'e mut ErrorGen,
...
impl<'a, 'b, 'c, 'd, 'e> InstrGeneratorComponent<'a, 'b, 'c, 'd, 'e> {
    pub fn new(
        emitter: VisitingEmitterComponent<'a, 'b, 'c, 'd>,
        ast: SimpleAST,
        err: &'e mut ErrorGen,
    ) -> Self {
        Self {
    ...
    pub fn run(&mut self) -> bool {
        ...
        while first_instr || self.emitter.next_instr() {
            ...
            rules.iter().for_each(|rule| {
                // Check if any of the configured rules match this instruction in the application.
                if let Some(loc_info) = get_loc_info_comp(rule, &self.emitter) {
                    if loc_info.num_alt_probes > 1 {
                        self.err
                            .multiple_alt_matches(self.emitter.curr_instr_name().as_str());
                    }
...
impl InstrGeneratorComponent<'_, '_, '_, '_, '_> {
    fn emit_probe(&mut self) -> bool {
        let mut is_success = true;
