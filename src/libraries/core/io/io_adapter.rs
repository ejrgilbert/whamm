...
impl IOAdapter {
    ...
    pub fn putsln<'a, T: Opcode<'a> + MacroOpcode<'a> + AddLocal>(&mut self, s: String, func: &mut T, err: &mut ErrorGen) {
    ...
    pub fn puts<'a, T: Opcode<'a> + MacroOpcode<'a> + AddLocal>(&mut self, s: String, func: &mut T, err: &mut ErrorGen) {
        // s -> [u8] (no need for data segment!)
        // iterate over and call putc
        let data = s.as_bytes();
        for c in data.iter() {
    ...
    pub fn putln<'a, T: Opcode<'a> + MacroOpcode<'a> + AddLocal>(&mut self, func: &mut T, err: &mut ErrorGen) {
        self.puts("\n".to_string(), func, err)
    }

    pub fn call_puti<'a, T: Opcode<'a> + MacroOpcode<'a> + AddLocal>(&mut self, func: &mut T, err: &mut ErrorGen) {
        self.call(PUTI, func, err);
    }

    fn putc<'a, T: Opcode<'a> + MacroOpcode<'a> + AddLocal>(&mut self, c: u8, func: &mut T, err: &mut ErrorGen) {
        func.i32_const(c as i32);
        self.call_putc(func, err);
    }

    pub fn call_putc<'a, T: Opcode<'a> + MacroOpcode<'a> + AddLocal>(&mut self, func: &mut T, err: &mut ErrorGen) {
