...
/// visitor methods on each sub-expression.  Returns `T::default()` for
/// variants that have no meaningful return value.
///
/// Visitors that only need to recurse (e.g. alias-resolution in the symbol
/// table builder) can use this as their default `visit_expr` implementation,
/// handling only the variant(s) they care about before delegating here.
pub fn traverse_expr_mut<T: Default, V: WhammVisitorMut<T>>(visitor: &mut V, expr: &mut Expr) -> T {
    match expr {
        Expr::UnOp { expr, .. } => {
