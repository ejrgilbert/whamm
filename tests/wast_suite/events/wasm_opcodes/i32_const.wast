...
;; ----------------------
;; ==== unpredicated ====
;; WHAMM --> i32 count; wasm:opcode:i32_const:before { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:i32_const:before /imm0 == 4/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:i32_const:before /imm0 == 0/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:i32_const:before /fid == 2 && pc == 8/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:i32_const:before /fid == 2/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:i32_const:before /fname == "basic_br" && pc == 8/ { count++; }
(assert_return (invoke "get_count") (i32.const 1))
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
...
;; after mode
;; WHAMM --> i32 count; wasm:opcode:i32_const:after /fname == "basic_br" && pc == 8/ { count++; }
(assert_return (invoke "get_count") (i32.const 1))
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
