...
;; ----------------------
;; ==== unpredicated ====
;; WHAMM --> i32 count; wasm:opcode:local_set:before { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:local_set:before /imm0 == 3/ { count++; }
(assert_return (invoke "get_count") (i32.const 0)) ;; location DNE
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
(assert_return (invoke "get_global_var1") (i32.const 3)) ;; sanity check
;; WHAMM --> i32 count; wasm:opcode:local_set:before /imm0 == 0/ { count++; }
(assert_return (invoke "get_count") (i32.const 0)) ;; location DNE
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
...
;; WHAMM --> i32 count; wasm:opcode:local_set:before /imm0 == 1/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:local_set:before /fid == 2 && pc == 1/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:local_set:before /fid == 2/ { count++; }
(assert_return (invoke "get_count") (i32.const 1))
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
...
;; WHAMM --> i32 count; wasm:opcode:local_set:before /fname == "basic_br" && pc == 1/ { count++; }
(assert_return (invoke "get_count") (i32.const 1))
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
...
;; after mode
;; WHAMM --> i32 count; wasm:opcode:local_set:after /fname == "basic_br" && pc == 1/ { count++; }
(assert_return (invoke "get_count") (i32.const 1))
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
