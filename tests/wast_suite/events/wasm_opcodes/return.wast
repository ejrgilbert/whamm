...
;; ----------------------
;; ==== unpredicated ====
;; WHAMM --> i32 count; wasm:opcode:_return:before { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:_return:before /fid == 2 && pc == 1/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:_return:before /fid == 2 && pc == 9/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:_return:before /fid == 2 && pc == 12/ { count++; }
...
;; WHAMM --> i32 count; wasm:opcode:_return:before /fname == "basic_br" && pc == 9/ { count++; }
(assert_return (invoke "get_count") (i32.const 1))
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
...
;; after mode
;; WHAMM --> i32 count; wasm:opcode:_return:after { count++; }
(assert_return (invoke "get_count") (i32.const 0))
;; @passes_uninstr
(assert_return (invoke "get_global_var0") (i32.const 0)) ;; sanity check
;; @passes_uninstr
